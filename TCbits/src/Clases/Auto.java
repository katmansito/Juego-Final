...
public class Auto {
    ...
    public Auto(int startX, int startY, String marca) {
        this(startX, startY, marca, "default");
    }

    // Sobrecarga para especificar circuito
    public Auto(int startX, int startY, String marca, String circuito) {
        this.x = startX;
        this.y = startY;
        this.velocidad = 5;
        this.marca = marca;

        // Ángulo inicial por defecto
        this.angulo = 180;

        // Si es el circuito de Toay, invertir orientación
        if (circuito.equalsIgnoreCase("Toay")) {
            this.angulo = 0; // o 0° si en Toay aparece al revés
        }
    ...
    public void actualizar(LimitesCircuitos limites) {
        ...
        // Detección de colisión con muro
        LimitesCircuitos.Terreno terrenoCentro = limites.detectarTerreno(
            (int) (x + getAncho() / 2.0),
            (int) (y + getAlto() / 2.0)
        );

        if (terrenoCentro == LimitesCircuitos.Terreno.MURO) {
            // volver a la posición anterior
            x = xAnterior;
            y = yAnterior;

            // Retroceder un poco adicional para evitar quedar atrapado
            double retroceso = 4.0; // podés probar 3–6 px
            x -= retroceso * Math.cos(Math.toRadians(angulo));
            y -= retroceso * Math.sin(Math.toRadians(angulo));

            // Rebote leve y pérdida de velocidad
            velocidad = -velocidad * 0.4;

            // Evitar oscilación mínima
            if (Math.abs(velocidad) < 0.2) velocidad = 0;
        }
